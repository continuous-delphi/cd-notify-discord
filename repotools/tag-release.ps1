...
# Creates and pushes a vX.Y.Z release tag
# Requires: PowerShell 7+, git (on PATH)
#
# Usage:
#   pwsh tools/tag-release.ps1 -Version 1.0.0
#   pwsh tools/tag-release.ps1 -ShowVersion
#
...
param(
  [Parameter(Mandatory=$true, ParameterSetName='Release', HelpMessage='Semantic version to tag, e.g. 1.0.0')]
  [ValidatePattern('^[0-9]+\.[0-9]+\.[0-9]+$')]
  [string] $Version,

  [Parameter(ParameterSetName='Release', HelpMessage='Skip the branch check (use when hotfixing from a non-default branch)')]
  [switch] $SkipBranchCheck,

  [Parameter(Mandatory=$true, ParameterSetName='ShowVersion', HelpMessage='Display the current script version and exit')]
  [switch] $ShowVersion
)
...
if ($ShowVersion) {
  ...
  if ($content -match '\$script:ToolVersion\s*=\s*''([^'']+)''') {
    ...
    Write-Error "Could not determine version from $scriptFile"
  }
  return
}
...
Write-Host ""
Write-Host "cd-notify-discord" -ForegroundColor White
