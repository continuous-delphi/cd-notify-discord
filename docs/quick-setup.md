...
GitHub suppresses events generated by its built-in `GITHUB_TOKEN`. If your
releases are created by a GitHub Actions workflow (rather than manually through
the GitHub UI), the release event will not trigger `cd-notify-discord.yml` and
no notification will be sent.

To fix this, create a fine-grained Personal Access Token (PAT) with
**Contents: Read and write** permission on the repository and store it as a
repository secret named `RELEASE_GH_TOKEN`. Then update the release step in
your workflow to use it:
...
**Alternative:** If you do not want to create a PAT, enable tag creation
notifications instead (`CD_NOTIFY_DISCORD_CREATE=tag`). A tag is always
created before a release and fires reliably regardless of how the release
is published.
